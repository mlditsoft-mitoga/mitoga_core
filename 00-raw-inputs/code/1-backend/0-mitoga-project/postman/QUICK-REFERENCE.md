...
> **TL;DR:** Referencia rápida de comandos y endpoints más usados - v1.2.0 con CRUD completo
...
newman run Mitoga-API.postman_collection.json `
  ...
  --folder "1. Catálogos Recursivos (POST only)"
```
...
## 📋 Endpoints Quick List (8 total)

### 🔍 Catálogos - Consultas (4)

**1.1. Buscar Árbol** `POST /catalogos/buscar-arbol`
```json
{ "tipo": "categorias_tutorias", "soloActivos": true }
```

**1.2. Buscar Ancestros** `POST /catalogos/buscar-ancestros`
```json
{ "catalogoId": "{{catalogoId}}", "incluirPropio": true }
```

**1.3. Buscar Descendientes** `POST /catalogos/buscar-descendientes`
```json
{ "catalogoId": "{{catalogoId}}", "incluirPropio": false }
```

**1.4. Buscar por Nombre** `POST /catalogos/buscar`
```json
{ "tipo": "categorias_tutorias", "nombre": "álgebra" }
```

### ✏️ Catálogos - CRUD (4) ⭐ NUEVO
...
**1.8. Eliminar (Soft Delete)** `POST /catalogos/eliminar` → HTTP 200
```json
{
  "catalogoId": "{{catalogoId}}",
