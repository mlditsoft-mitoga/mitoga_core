...
public class Catalogo {
    ...
    /**
     * Actualiza el nombre en inglés del catálogo.
     */
    public void actualizarNombreEn(String nuevoNombreEn) {
        this.nombreEn = nuevoNombreEn != null ? nuevoNombreEn.trim() : null;
        this.updatedAt = LocalDateTime.now();
    }

    /**
     * Actualiza la descripción del catálogo.
     */
    ...
    /**
     * Actualiza la descripción en inglés del catálogo.
     */
    public void actualizarDescripcionEn(String nuevaDescripcionEn) {
        this.descripcionEn = nuevaDescripcionEn != null ? nuevaDescripcionEn.trim() : null;
        this.updatedAt = LocalDateTime.now();
    }

    /**
     * Actualiza el icono del catálogo.
     */
    public void actualizarIcono(String nuevoIcono) {
        this.icono = nuevoIcono != null ? nuevoIcono.trim() : null;
        this.updatedAt = LocalDateTime.now();
    }

    /**
     * Actualiza el color del catálogo.
     */
    ...
    /**
     * Actualiza el código del catálogo.
     */
    public void actualizarCodigo(String nuevoCodigo) {
        Objects.requireNonNull(nuevoCodigo, "Nuevo código no puede ser nulo");
        if (nuevoCodigo.trim().isEmpty()) {
            throw new IllegalArgumentException("Nuevo código no puede estar vacío");
        }
        if (nuevoCodigo.length() > 50) {
            throw new IllegalArgumentException("Nuevo código no puede exceder 50 caracteres");
        }
        if (!nuevoCodigo.matches("^[A-Z0-9_-]+$")) {
            throw new IllegalArgumentException(
                    "El código solo puede contener letras mayúsculas, números, guiones y guiones bajos");
        }
        this.codigo = nuevoCodigo.trim();
        this.updatedAt = LocalDateTime.now();
    }

    /**
     * Cambia el padre del catálogo.
     * NOTA: El nivel y path se recalcularán automáticamente por el trigger de BD.
     */
    public void cambiarPadre(CatalogoId nuevoPadreId) {
        if (this.nivel == 0) {
            throw new IllegalStateException("No se puede cambiar el padre de un nodo raíz");
        }
        this.padreId = nuevoPadreId;
        this.updatedAt = LocalDateTime.now();
    }
