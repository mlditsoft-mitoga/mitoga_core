...
public class CatalogoController {
        ...
        private final CrearCatalogoService crearCatalogoService;
        private final ActualizarCatalogoService actualizarCatalogoService;
        private final ObtenerCatalogoPorIdService obtenerCatalogoPorIdService;
        private final EliminarCatalogoService eliminarCatalogoService;
         ...
         */
        @Operation(summary = "Crear un nuevo catálogo", description = "Crea un nuevo nodo en el árbol de catálogos. "
                        + "Puede ser un nodo raíz (sin padre) o un nodo hijo. **Usar POST con body JSON.**")
        @ApiResponses(value = {
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Catálogo creado exitosamente", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "Request inválido"),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409", description = "Ya existe un catálogo con ese código")
        })
        @PostMapping("/crear")
        public ResponseEntity<ApiResponse<CatalogoResponse>> crearCatalogo(
                        @Valid @RequestBody CrearCatalogoRequest request) {
                log.info("POST /api/v1/catalogos/crear - tipo: {}, código: {}",
                                request.tipo(), request.codigo());

                com.mitoga.shared.domain.catalogo.CatalogoId padreId = request.padreId() != null
                                ? new com.mitoga.shared.domain.catalogo.CatalogoId(request.padreId())
                                : null;

                CatalogoResponse catalogo = crearCatalogoService.execute(
                                request.tipo(),
                                padreId,
                                request.codigo(),
                                request.nombre(),
                                ...
                                request.color(),
                                request.activo(),
                                request.seleccionable(),
                                request.metadatos());

                return ResponseEntity.ok(ApiResponse.success(catalogo, SuccessMessage.GEN_CREATED));
        }

        /**
         * POST /api/v1/catalogos/obtener-por-id
         * 
         * Obtiene un catálogo por su ID.
         */
        @Operation(summary = "Obtener catálogo por ID", description = "Retorna un catálogo específico por su identificador. **Usar POST con body JSON.**")
        @ApiResponses(value = {
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Catálogo obtenido exitosamente", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "Catálogo no encontrado")
        })
        @PostMapping("/obtener-por-id")
        public ResponseEntity<ApiResponse<CatalogoResponse>> obtenerCatalogoPorId(
                        @Valid @RequestBody ObtenerCatalogoPorIdRequest request) {
                log.info("POST /api/v1/catalogos/obtener-por-id - ID: {}", request.catalogoId());

                com.mitoga.shared.domain.catalogo.CatalogoId catalogoId = new com.mitoga.shared.domain.catalogo.CatalogoId(
                                request.catalogoId());

                CatalogoResponse catalogo = obtenerCatalogoPorIdService.execute(catalogoId);

                return ResponseEntity.ok(ApiResponse.success(catalogo, SuccessMessage.GEN_RETRIEVED));
        }
         ...
         */
        @Operation(summary = "Actualizar catálogo", description = "Modifica los atributos de un catálogo existente. "
                        + "Solo actualiza los campos que vienen informados (parcial update). **Usar POST con body JSON.**")
        @ApiResponses(value = {
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Catálogo actualizado exitosamente", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "Catálogo no encontrado"),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409", description = "Conflicto con el nuevo código")
        })
        ...
        public ResponseEntity<ApiResponse<CatalogoResponse>> actualizarCatalogo(
                ...
                log.info("POST /api/v1/catalogos/actualizar - ID: {}", request.catalogoId());
                ...
                CatalogoResponse catalogo = actualizarCatalogoService.execute(
                                catalogoId,
                                padreId,
                                request.codigo(),
                                request.nombre(),
                ...
                return ResponseEntity.ok(ApiResponse.success(catalogo, SuccessMessage.GEN_UPDATED));
        }
         ...
         */
        @Operation(summary = "Eliminar catálogo", description = "Desactiva un catálogo (soft delete). "
                        + "Opcionalmente puede eliminar también todos sus descendientes en cascada. **Usar POST con body JSON.**")
        @ApiResponses(value = {
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Catálogo eliminado exitosamente", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "Catálogo no encontrado"),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409", description = "El catálogo tiene hijos y no se especificó eliminarlos")
        })
        @PostMapping("/eliminar")
        public ResponseEntity<ApiResponse<Void>> eliminarCatalogo(
                        @Valid @RequestBody EliminarCatalogoRequest request) {
                log.info("POST /api/v1/catalogos/eliminar - ID: {}, cascada: {}",
                                request.catalogoId(), request.eliminarDescendientes());

                com.mitoga.shared.domain.catalogo.CatalogoId catalogoId = new com.mitoga.shared.domain.catalogo.CatalogoId(
                                request.catalogoId());

                eliminarCatalogoService.execute(catalogoId, request.eliminarDescendientes());

                return ResponseEntity.ok(ApiResponse.success(null, SuccessMessage.GEN_DELETED));
        }
}
