...
RETURNS VOID AS $$
DECLARE
    full_table_name TEXT := 'tutores_schema.' || p_table_name;
...
BEGIN
    ...
    IF NOT shared_schema.tabla_tiene_columna('tutores_schema', p_table_name, 'updated_at') THEN
        ...
        RAISE NOTICE 'Columna updated_at agregada a %', full_table_name;
    END IF;
    
    -- Crear trigger si no existe (Lección #4: verificación defensiva)
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers 
                   WHERE trigger_schema = 'tutores_schema' 
                   AND event_object_table = p_table_name
                   AND trigger_name = v_trigger_name) THEN
        EXECUTE format('CREATE TRIGGER %I BEFORE UPDATE ON %I.%I FOR EACH ROW EXECUTE FUNCTION shared_schema.trigger_set_timestamp()', 
                      v_trigger_name, 'tutores_schema', p_table_name);
        RAISE NOTICE 'Trigger % creado para %', v_trigger_name, full_table_name;
    ELSE
        RAISE NOTICE 'Trigger % ya existe para %', v_trigger_name, full_table_name;
    END IF;
END;
$$ LANGUAGE plpgsql;
